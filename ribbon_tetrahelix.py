...
WALL: PER-FACE PRISMS, NOT A VERTEX OFFSET
------------------------------------------
The obvious shell is to offset every vertex along its own normal by +-wall/2. It is
wrong here, and quietly so. At a vertex of this surface the incident face normals
spread up to 143 deg, so their mean comes out very nearly RADIAL -- while the faces
themselves sit far off radial. Offsetting radially by 2.4 mm therefore buys only
2.4*cos(theta) of wall perpendicular to each face, which measured out at 1.26 mm: a
wall about half its nominal, and a volume 47% under area x thickness.

So each face gets its own inward prism instead, giving exactly `wall` perpendicular
everywhere by construction. That is only safe because every fold on this surface is
convex from outside (148.41, 141.06 and 70.53 deg, all under 180), so neighbouring
prisms OVERLAP along every fold rather than leaving a gap -- asserted, not assumed.

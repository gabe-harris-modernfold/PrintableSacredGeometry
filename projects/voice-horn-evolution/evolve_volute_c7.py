...
R1 = 35.0
#              r_t   r_m    L      k     H
XL = np.array([13.0, 50.0, 300.0, 0.08, 40.0])
XU = np.array([30.0, 123.0, 900.0, 0.30, 200.0])
...
Fp, Xp = F[pf], X[pf]
print(f"cycle 7 (VOLUTE) front: {len(pf)} feasible of {POP}")
...
# hand Volute (Rev-1 doc): rt=26.5 equiv (2199 mm2), rm=122.5 equiv, L=492, k=0.154, H=110
Fh, cvh, Gh = evaluate(np.array([[26.5, 122.5, 492.0, 0.154, 110.0]]))
...
np.savetxt("pareto_c7.csv", np.hstack([Xp, -Fp[:, :1], Fp[:, 1:], fc[:, None]]), delimiter=",",
           header="rt,rm,L,k,H,G_dB,refl,Pcost,fc_Hz", comments="")
...
ax.grid(alpha=0.15)
fig.suptitle("THE FORGE, cycle 7  -  VOLUTE family (spiral packing gate + corrected evaluator)",
             fontsize=12, fontweight="bold", color="#1F2937", x=0.06, ha="left")
fig.savefig("evolution_c7.png", dpi=150)
print("wrote pareto_c7.csv, evolution_c7.png")

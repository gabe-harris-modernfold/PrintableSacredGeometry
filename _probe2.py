...
# only cast against near-axis triangles: a thin feature's opposite wall is also near-axis
targets=[d for d in data if d[5]<24]
def hit(o,d,sa,sb,sc):
    best=1e18
    for (a,b,c,n,cen,md) in targets:
        if a==sa and b==sb and c==sc:continue
    ...
    return best
ths=[];minrr=min(d[5] for d in data)
for (a,b,c,n,cen,md) in data:
    if md>16:continue
    o=(cen[0]-1e-4*n[0],cen[1]-1e-4*n[1],cen[2]-1e-4*n[2])
    t=hit(o,(-n[0],-n[1],-n[2]),a,b,c)
    if t<1e17: ths.append(t)
ths.sort()
print("min radius from axis (throat hole):",round(minrr,3),"mm")
print("near-throat probes",len(ths),"| min wall",round(ths[0],3),"mm",
